...
void MainLighting_half(half3 positionWS, half3 normalWS, half3 viewWS, half smoothness, out half specular, out half diffuse)
{
    ...
    diffuse = 0;
        ...
        half diffuse_hard = smoothstep(0.005,0.01,diffuse);
        ...
        half specular_hard = smoothstep(0.005,0.01,specular);
...
void MainLighting_float(float3 positionWS, float3 normalWS, float3 viewWS, float smoothness, out float specular, out float diffuse)
{
    MainLighting_half(positionWS, normalWS, viewWS, smoothness, specular, diffuse);
}
...
void AdditionalLighting_float(float3 positionWS, float3 normalWS, float3 viewWS, float smoothness, float hardness, out float3 specular, out float3 diffuse)
{
    AdditionalLighting_half(positionWS, normalWS, viewWS, smoothness, hardness, specular, diffuse);
}
